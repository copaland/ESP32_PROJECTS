...
/*
    ...
    if (Firebase.RTDB.setInt(&fbData, "test/int", count)){
...
if (Firebase.RTDB.getInt(&fbData, "/test/int")) {
  if (fbData.dataType() == "int") {
    intValue = fbData.intData();
...
if (Firebase.RTDB.getFloat(&fbData, "/test/float")) {
  if (fbData.dataType() == "float") {
    floatValue = fbData.floatData();
...
//---------------
  if (Firebase.get(fbData,"/AutoHome/S1")) {
    if (fbData.dataType() == "string") {
      String state1 = fbData.stringData();
      if (state1=="1"){
        digitalWrite(LED_1,HIGH); //Device1 is ON
  ...
  if (Firebase.get(fbData,"/AutoHome/S2")) {
    if (fbData.dataType() == "string") {
      String state2 = fbData.stringData();
	  if (state2=="1"){
	    digitalWrite(LED_2,HIGH);//Device2 is ON
  ...
  if (Firebase.get(fbData,"/AutoHome/S3")) {
    if (fbData.dataType() == "string") {
      String state3 = fbData.stringData();
      if (state3=="1"){
        digitalWrite(LED_3,HIGH);//Device3 is ON
  ...
  if (Firebase.get(fbData,"/AutoHome/S4")) {
    if (fbData.dataType() == "string") {
      String state4 = fbData.stringData();
      if (state4=="1"){
        digitalWrite(LED_4,HIGH);//Device4 is ON
